...
// initial conditions
if($race == Protoss) {
	$product = Product::byName("Nexus");
	$timeline->spellcasters->add(new Spellcaster($product, 0));
	$timeline->farms->add(new Farm(0, $product->supplyCapacity));
} elseif($race == Terran) {
	...
	InfestedCCs::$debug = (bool)($debugFlags & InfestedCCs::debugFlag);
	$timeline->infestedccs->add(new InfestedCC(0, 3));
	$timeline->farms->add(new Farm(0, $InfCommandCenter->supplyCapacity));
	$timeline->farms->add(new Farm(0, $InfOverlord->supplyCapacity));
} */
$timeline->queues->add(new ProductionQueue(Product::designated($race | Base)));
//if($race == Stukov) {
//	$timeline->supplyCount = 12;
//} else {
...
if($race == Zerg) {
	...
	?>
	<h3>Charges generated</h3>
	<p>This table shows the charges generated by your Infested CCs.</p>
	<?php
	$timeline->infestedccs->update($timeEnds);
	echo (string)$timeline->infestedccs;
}
